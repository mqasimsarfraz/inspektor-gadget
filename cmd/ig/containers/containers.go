...
func NewListContainersCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(*cobra.Command, []string) error {
			...
			if !optionWatch {
				parser, err := commonutils.NewGadgetParserWithK8sAndRuntimeInfo(&commonFlags.OutputConfig, columnVisibility(containercollection.GetColumns()))
				if err != nil {
			...
			cols := columnVisibility(columns.MustCreateColumns[containercollection.PubSubEvent]())
...
func columnVisibility[T containercollection.Container | containercollection.PubSubEvent](cols *columns.Columns[T]) *columns.Columns[T] {
