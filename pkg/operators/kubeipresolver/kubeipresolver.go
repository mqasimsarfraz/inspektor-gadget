...
func (k *KubeIPResolver) InstantiateDataOperator(gadgetCtx operators.GadgetContext, instanceParamValues api.ParamValues) (operators.DataOperatorInstance, error) {
	...
	for _, ds := range gadgetCtx.GetDataSources() {
		...
		if len(endpoints) > 0 {
			...
			for _, ep := range endpoints {
				...
				k8sKindAcc, err := k8sSubAcc.AddSubField("kind", api.Kind_String, datasource.WithFlags(datasource.FieldFlagHidden))
				if err != nil {
	...
	inst.endpointsAccessors = epAccessors
	return inst, nil
}
	...
	for ds, acc := range m.endpointsAccessors {
		for _, a := range acc {
			ds.Subscribe(func(source datasource.DataSource, data datasource.Data) error {
				ips := a.root.GetSubFieldsWithTag("type:" + ipAddrType)
				if len(ips) != 1 {
					return fmt.Errorf("expected %d %q field, got %d", 1, ipAddrType, len(ips))
				}
				if ips[0].Size() != 16 {
					return fmt.Errorf("expected %q to have 16 bytes, got %d", ipAddrType, ips[0].Size())
				}

				version := a.root.GetSubFieldsWithTag("name:version")
				if len(version) != 1 {
					return fmt.Errorf("expected %d %q field, got %d", 1, "version", len(version))
				}

				ip := ips[0].Get(data)
				if ip == nil {
					return fmt.Errorf("expected %q to have a value", ipAddrType)
				}
				v, err := version[0].Uint8(data)
				if err != nil {
					return fmt.Errorf("expected %q to have a value", "version")
				}
				var addrStr string
				switch v {
				case 4:
					addrStr = net.IP(ips[0].Get(data)[:4]).String()
				case 6:
					addrStr = net.IP(ips[0].Get(data)).String()
				default:
					return fmt.Errorf("unsupported IP version %d", v)
				}
...
func init() {
	...
	// TODO: use localmanager.EnrichWithK8sApiserver instead to control this for ig,
	// once we support sharing params. For now, we only support this operator in Kubernetes.
