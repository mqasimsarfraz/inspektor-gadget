...
func newTraceDNSStep(t *testing.T, tc testCase) (igtesting.TestStep, []igtesting.Option) {
	...
	commonDataOpts := []utils.CommonDataOption{utils.WithContainerImageName(tc.clientImage), utils.WithContainerID(clientContainer.ID())}
...
func TestTraceDNSTCP(t *testing.T) {
	...
	tc := testCase{
		...
		clientUID:         0,
		clientGID:         0,
		clientExpectedCmd: "isc-net-0000", // one of the thread of nslookup
